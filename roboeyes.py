...
if __name__ == "__main__":
    ...
    eyes.begin(device.width, device.height, 30) # 30 FPS
    ...
    eyes.open() # Ensure eyes are open initially

    print("RoboEyes started. Press Ctrl+C to exit.")

    try:
        while True:
            # Update the eye animations and drawing
            eyes.update()

            # --- Example animation sequence ---
            # You can uncomment these to see different animations

            # # Default mood and position
            # eyes.setMood(DEFAULT)
            # eyes.setPosition(DEFAULT)
            # for _ in range(60): eyes.update() # Run for 2 seconds (30 frames/sec * 2 sec)

            # # Happy mood
            # eyes.setMood(HAPPY)
            # for _ in range(60): eyes.update()
            ...
            # # Blinking
            # eyes.setMood(DEFAULT)
            # eyes.blink() # This is a quick one-shot, relies on `update` to complete
            # for _ in range(30): eyes.update() # Allow animation to finish

            # # Confused animation
            # eyes.anim_confused()
            # for _ in range(60): eyes.update() # Allow animation to play out

            # # Laugh animation
            # eyes.anim_laugh()
            # for _ in range(60): eyes.update() # Allow animation to play out

            # # Eyes looking around (manual position changes)
            # eyes.setPosition(N)
            # for _ in range(30): eyes.update()
            # eyes.setPosition(SE)
            # for _ in range(30): eyes.update()
            # eyes.setPosition(W)
            # for _ in range(30): eyes.update()
            ...
            # # Autoblinker and Idle mode (run in background)
            # eyes.setAutoblinker(ON, 1, 3) # Blink every 1-4 seconds
            # eyes.setIdleMode(ON, 2, 5) # Move eyes every 2-7 seconds
            # eyes.setMood(DEFAULT)
            # eyes.setCuriosity(True) # Turn on curious mode
            # for _ in range(300): # Run for 10 seconds to see random animations
            #     eyes.update()
            # eyes.setAutoblinker(OFF)
            # eyes.setIdleMode(OFF)
            # eyes.setCuriosity(False)

            # Keep a base animation loop if nothing else is active
            eyes.setMood(DEFAULT)
            eyes.setPosition(DEFAULT)
            time.sleep(0.01) # Small delay to avoid busy-waiting if nothing else is animating
