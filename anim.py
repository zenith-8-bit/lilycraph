...
# Changed from spi to i2c for I2C communication
...
# I2C on Raspberry Pi typically uses bus 1.
# Common I2C address for 128x64 SSD1306 OLEDs is 0x3C.
I2C_BUS = 1
I2C_ADDRESS = 0x3C

# For I2C, DC and RST pins are NOT typically used for the display itself,
# they are handled internally by the I2C chip or not broken out.
# So, we remove these parameters from the serial initialization.

# --- OLED Device Initialization ---
device = None # Initialize device to None
try:
    # Attempt to initialize SSD1306 via I2C
    # Removed gpio_DC and gpio_RST as they are not used for I2C
    serial = i2c(port=I2C_BUS, address=I2C_ADDRESS)
    device = ssd1306(serial)
    print(f"OLED display initialized successfully: {device.width}x{device.height} via I2C.")
except Exception as e:
    # If I2C device is not found, print a clear message and exit.
    print(f"ERROR: Could not initialize OLED display via I2C. Please ensure:")
    print(f"1. I2C is enabled on your Raspberry Pi ('sudo raspi-config' -> Interface Options -> I2C).")
    print(f"2. Your user has permissions to access I2C (try 'sudo adduser $USER i2c' and reboot).")
    print(f"3. The OLED is correctly wired: SDA to GPIO 2 (Physical Pin 3), SCL to GPIO 3 (Physical Pin 5).")
    print(f"4. The I2C address (0x{I2C_ADDRESS:X}) is correct for your display (use 'i2cdetect -y {I2C_BUS}' to check).")
    print(f"Details: {e}")
